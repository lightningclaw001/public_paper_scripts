...
params.control = 0.05; %C-rate = 0.1. -0.1 is charge. +0.1 is discharge
params.control = params.control*0.53;
...
params_nondim.mu_res = -params.mu_res*params_nondim.e/params_nondim.kBT + params_nondim.muR_ref;
...
params_nondim.mu_end_low = -20; %0 is 0.26, 9 is 0.36
params_nondim.mu_end_up = 40; %0.89 voltage--31
...
sol = ode15s(@(t,f) dfdt_volt(t, f, params, params_nondim), [0,params.max_cap/params_nondim.control_abs/2], IC, opts);
x = linspace(0, sol.x(end), 500);
y = deval(sol, x);
y = [y; ones(1,500)*params_nondim.mu_res];
...
save('Ni' + string(params_nondim.x) + '_volt_3_cycle_' + string(params.mu_res) + 'V.mat')
...
value = [params.vf*y(1:params.c_s_ind);
    params.vf*y(1:params.c_s_ind)-1];
...
function dy = dfdt_volt(t, y, params, params_nondim)
