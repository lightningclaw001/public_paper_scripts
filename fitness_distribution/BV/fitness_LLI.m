...
%params.order = 1;
params.order = 3;
params.k = 0.003; %here k is the degradation parameter for electrolyte 
...
function [k_array, rxn, eta] = R_LLI(c_grid, c_lyte, mures, k0, rxn_method, params)
%R is dc/dt, from the population balance equation. can be BV or linear
%set symmetry coefs
alpha = 0.5;
%get overpotential
...
switch rxn_method
    ...
    case "BV"
        %using the transition state rxn model
        rxn = k_array.*(1-c).*(c./(1-c)).^alpha.*c_lyte.^(1-alpha).*(exp(-alpha*eta)-...
            exp((1-alpha)*eta)).*(tanh(-((abs(eta))-8))+1)/2;
        if isa(eta,'casadi.MX') || isa(eta,'casadi.SX')
            eta = if_else(eta==0,1e-16,eta);
        else
    ...
    case "Marcus"
        lmbda = params.lambda;
        %Marcus model
        rxn = k_array.*(1-c).*((c_lyte.*exp(-power(lmbda+etaf,2)./(4*lmbda))) ...
          - (c.*exp(-power(lmbda-etaf,2)./(4*lmbda))));
        if isa(eta,'casadi.MX') || isa(eta,'casadi.SX')
            eta = if_else(eta==0,1e-16,eta);
        else
    ...
    case "CIET"
        ...
        i_red = helper_fun(-etaf, params.lambda);
        i_ox = helper_fun(etaf, params.lambda);
        rxn = k_array.*(1-c).*(c_lyte.*i_red - c.*i_ox);
        if isa(eta,'casadi.MX') || isa(eta,'casadi.SX')
            eta = if_else(eta==0,1e-16,eta);
        else
            eta(eta==0) = 1e-16;
...
elseif params.order == -2
    ...
    out = 1./params.R_grid_1*1./(1-omega.*di-omega.^2.*0.5.*(di.^2+d2ideta2(c_grid, mures, k0, ...
        rxn_method, params).'));
elseif params.order == -20
    %this is MHC
    out = 1./params.R_grid_1.*c_lyte.^0.5;
% elseif params.order == -21
%     %this is MHC
%     di = dideta(c_grid, mures, k0, ...
%         rxn_method, params).';
%     out = 1./params.R_grid_1.*(1-1./Rxn_array.'.*(1-1./c_lyte).*(di+));
% end
end
end
...
function [df_dt, y] = dfdt_voltage(t, y, mures, params, rxn_params)
...
%         params.delta_c*y_array(params.N*i)/2)*0;
%         ];
% end
%     
...
% domegadt_rescaled = domegadt./min(domegadt);
% indexes = (domegadt_rescaled < params.cutoff);
% domegadt = domegadt.*indexes;
%domegadt(domegadt_rescaled > params.cutoff) = 0;
...
%for numerical efficiency, where f is very small, we can drop the W terms
F_c = avg_2D(f.*Rxn_array.'./params.R_grid_1,params.N,params.M);
%pad with zeros and add diffusive term
F_c = F_c - rxn_params.D0*avg_2D(k_array,params.N,params.M).* ...
...
function rxn = R_resistance(mures, R_intercalation, Omega, k, params)
...
%             toc()
            ...
            params.total_time = params.total_time + sol.x(end);
